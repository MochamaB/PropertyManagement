    ...
    <!--------------   IF THERES NO DATA  ------------>
                ...
                <div class="table-responsive">
                     ...
                     <table id="table"
                                ...
                                <th data-sortable="true">No Of Work Orders</th>
                                <th>Actions</th>
                ...
                @if (Route::currentRouteName() == 'YearViewrepairwork')
                        ...
                        @foreach($workorderyeargroup as $key => $item)
                                       <tr>
                                           <td>{{ $key+1 }}</td>
                                           <td>{{$item->year}}</td>
                                           <td>{{$item->noofworkorders}}</td> 
                                           <td>
                                           <a href="{{ url('MonthViewrepairwork/'.$item->year) }}" class="btn btn-primary btn-sm"><i class="mdi mdi-calendar-multiple"></i> Month View </a>
                    ...
                    @if (Route::currentRouteName() == 'MonthViewrepairwork')
                        ...
                        @foreach($workordermonthgroup as $key => $item)
                                       <tr>
                                           ...
                                           <td>{{$item->noofworkorders}}</td> 
                                           <td>
                                           <a href="{{ url('Viewrepairwork/'.$item->year. '/' . $item->month) }}" class="btn btn-primary btn-sm"><i class="mdi mdi-calendar-multiple"></i>View Details</a>
